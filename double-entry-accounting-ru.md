...
* Проблема: Физический подсчет кассы выявил недостачу, изначально учтенную как убыток.
...
> **Примечание:** Формулы в этом разделе приведены на английском языке, так как они должны использоваться именно в таком виде в Google Sheets. Описания и пояснения переведены на русский язык.
...
- ChartOfAccounts - именованный диапазон, включающий коды и наименования
...
Добавьте условное форматирование для выделения несбалансированных операций:
1. Выберите всю строку операции
2. Формат → Условное форматирование → Добавить правило
3. Правила форматирования:
   - Применить к диапазону: Выберите все столбцы в журнале операций
   - Формат ячеек, если... Пользовательская формула:
   ```=SUMIFS($DebitColumn, $TransactionIDColumn, $D2) <> SUMIFS($CreditColumn, $TransactionIDColumn, $D2)```
   - Стиль форматирования:
     - Красный фон: #F4CCCC
     - Красный текст: #990000
     - Жирный текст
4. Это выделит всю строку для любой операции, где дебет не равен кредиту
