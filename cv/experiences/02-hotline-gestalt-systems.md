...
- Real-time [LiveKit](livekit) [voice-agent pipeline](agents,product,backend) using Deepgram streaming speech recognition, Silero VAD, [Gemini](llms) and [OpenAI](llms) tool-calling, and ElevenLabs streaming speech synthesis. Inbound and outbound calls ([Quo, formerly OpenPhone](quo)), [SMS](twilio), and email communication. Used STT, TTS, and VAD to achieve a highly responsive voice agent with 200–500 ms latency while delegating tool calls to a background agent (a feature implemented by ChatGPT Voice only in July 2026).
- [Agent harness based on Vercel AI SDK](agent-harness,vercel-ai-sdk), with shared tools, persisted context, memory system, and [advanced context preparation and retrieval](search) for agents. [Scheduled execution and work queues](autonomous-development) ([PGMQ](postgres)), resilient agent orchestration and provisioning, and transactional side effects, [Redis caching](backend).
...
- Built [database migrations](postgres) and remote production tests across [SMS, email, and voice](product).
