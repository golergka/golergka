...
export function pdfBlocks(root) {
  function runs(node, style = {}) {
    if (node.nodeType === 3) return [{ text: node.textContent.replace(/\s+/g, " "), ...style }];
  ...
  for (const entry of root.querySelector("#cv-app").children) {
    ...
    for (const child of entry.children) {
      if (child.matches("h3")) blocks.push(block(child, 10, 2));
        ...
        blocks.at(-1).runs.push({text:"  ·  ", color:85}, ...runs(child, {color:85}));
      }
      else if (child.matches(".cv-summary")) blocks.push(block(child, 9, 2));
      else if (child.matches("ul")) {
        for (const li of child.children) blocks.push(block(li, 9, 1, { bullet: true }));
...
export function createCvPdf(jsPDF, groups, title) {
  ...
  pdf.setProperties({ title, author: "Max Yankov", subject: "Selected experience", creator: "golergka.com CV" });
  const margin = 26, width = pdf.internal.pageSize.getWidth() - margin * 2;
    ...
    for (const run of block.runs) {
      ...
      for (let text of run.text.split(/(\s+)/).filter(Boolean)) {
        ...
        while (text) {
          ...
          if (used && used + length > available) { lines.push([]); used = 0; }
          if (used || part.trim()) {
  ...
  const blockHeight = (block) => (block.rule ? 3 : block.lines.length * block.size * leadingRatio) + block.after;
  const layout = (scale) => groups.map((group) => group.map((source) => {
    const block = {...source, size: source.size * scale, after: source.after * scale};
  ...
  for (let scale = 0.98; laidOutGroups.flat().reduce((sum, block) => sum + blockHeight(block), 0) > bottom - margin && scale >= 7 / 9; scale -= 0.02) {
  ...
  for (const laidOut of laidOutGroups) {
    ...
    if (y > margin && y + height > bottom && height <= bottom - margin) { pdf.addPage(); y = margin; }
    for (const block of laidOut) {
      if (block.rule) {
        y += 3;
        pdf.setDrawColor(205); pdf.setLineWidth(0.5);
        if (y < bottom) pdf.line(margin, y, margin + width, y);
      } else {
        const leading = block.size * leadingRatio;
        for (let i = 0; i < block.lines.length; i++) {
          if (y + leading > bottom) { pdf.addPage(); y = margin; }
          let x = margin + (block.bullet ? 13 : 0);
          if (block.bullet && i === 0) {
            pdf.setFont("times", "normal"); pdf.setFontSize(block.size); pdf.setTextColor(0);
            pdf.text("•", margin, y + block.size);
          }
          for (const run of block.lines[i]) {
            pdf.setFont("times", run.bold ? "bold" : "normal"); pdf.setFontSize(block.size);
            if (run.mark) { pdf.setFillColor(255, 240, 166); pdf.rect(x, y + 1, run.width, block.size + 2, "F"); }
            if (run.href) pdf.setTextColor(0, 0, 180); else pdf.setTextColor(run.color ?? block.color ?? 0);
  ...
  for (let page = 1; pdf.getNumberOfPages() > 1 && page <= pdf.getNumberOfPages(); page++) {
    pdf.setPage(page); pdf.setFont("times", "normal"); pdf.setFontSize(7); pdf.setTextColor(110);
    pdf.text(`${page} / ${pdf.getNumberOfPages()}`, margin + width, bottom + 16, { align: "right" });
  }
  return pdf;
}
